...
class ProductControllerTest {
    ...
    void addProduct_success() throws Exception {
        ProductModel product = new ProductModel(1L, "Test Product", "Desc", "phone", 150.0, 5);
        when(productService.addProduct(any(ProductModel.class))).thenReturn(product);

        mockMvc.perform(post("/products")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"name\":\"Test Product\", \"description\":\"Desc\", \"category\":\"phone\", \"price\":150.0, \"quantity\":5}"))
    ...
    void getAllProducts_success() throws Exception {
        List<ProductModel> products = List.of(new ProductModel(1L, "Test Product", "Desc", "phone", 150.0, 5));
    ...
    void updateProduct_success() throws Exception {
        ProductModel updated = new ProductModel(1L, "Updated Product", "New Desc", "New phone", 350.0, 10);
        when(productService.updateProduct(eq(1L), any(ProductModel.class))).thenReturn(updated);

        mockMvc.perform(put("/products/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"name\":\"Updated Product\", \"description\":\"New Desc\", \"category\":\"New phone\", \"price\":350.0, \"quantity\":10}"))
                .andExpect(status().isOk())
    ...
    void getByPriceRange_success() throws Exception {
        List<ProductModel> products = List.of(new ProductModel(1L, "Test Product", "Desc", "phone", 150.0, 5));
        when(productService.getProductsByPriceRange(5.0, 200.0)).thenReturn(products);
    ...
    void getByCategory_success() throws Exception {
        List<ProductModel> products = List.of(new ProductModel(1L, "Test Product", "Desc", "phone", 150.0, 5));
        when(productService.getProductsByCategory("phone")).thenReturn(products);
